...
public class GSASearchEngine implements ISearchEngine {
	...
	public void search(StringTokenizer terms, ISearchParams parameters) throws Exception {
		...
		query = URLEncoder.encode(query, "UTF-8"); // Url encoding
	...
	private String getRequestSearchURL(ISearchParams parameters) {
		...
		for(PARAM name : parameters) {
			...
			if(PARAM.QUERY.equals(name)) {
				...
				//url.append("lr=lang_").append(value);
	...
	private class ResultsGSAXMLParser extends DefaultHandler {
		...
		private static final String TM = "TM"; // total time in seconds
		...
		public void endElement(String uri, String localName, String qName)
				throws SAXException {
			if(TM.equals(qName)) mResults.addMetadata(METADATA.TOTAL_TIME, this.sb.toString().trim());
			else if (RES_M.equals(qName)) // Estimated total number results
				mResults.addMetadata(METADATA.TOTAL_RESULTS, this.sb.toString().trim());
			else if(RESULT.equals(qName)) mResults.add(r);
			else if(RESULT_TITLE.equals(qName)) r.setTitle(this.sb.toString().trim());
			else if(RESULT_LINK.equals(qName)) {
				String url = this.sb.toString().trim();
				try {
					//LOGGER.severe("--SHortening url " + url);
			...
			else if(RESULT_SNIPPET.equals(qName)) r.setDescription(this.sb.toString().trim());
			else if(RESULT_LANG.equals(qName)) r.setLang(this.sb.toString().trim());
