...
<%@ include file="/WEB-INF/views/common/options_menu.jsp"%>
		...
		<table style="width:100%; border-spacing:0;">
			<thead>
				<tr>
					...
					<th><fmt:message key="course.actions"/></th>
				</tr>	
			</thead>
			<tbody>
			<c:forEach items="${paginator.collection}" var="course" varStatus="counter">
				<tr>	
					<td><a href="<c:url value='/${rc.locale.language}/extended/course/detail/${course.id}'/>" title="${course.title}">${course.title}</a></td>
					...
					<td>
						<div id="menu-item-${counter.count}" class="options-menu">		
							...
							<ul class="content-options-menu" style="display: none">
								...
								<li>
									<a class="edit-multilanguage-link" href="/extended/course/edit/${course.id}/" title="${course.title}">
								...
								<sec:authorize ifAllGranted="ROLE_MANUAL_MEDIATOR">
								<li>
									<a href="<c:url value='/${rc.locale.language}/extended/course/delete/${course.id}'/>" title="${course.title}">
		...
		<table id="operations">
			<tr>
				<sec:authorize ifAllGranted="ROLE_MANUAL_MEDIATOR">
				<td>
					<a href="<c:url value='/${rc.locale.language}/extended/course/'/>" title="<fmt:message key='course.action.add'/>">
